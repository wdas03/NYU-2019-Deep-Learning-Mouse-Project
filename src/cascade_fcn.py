...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cbe', num_groups=8,
                 **kwargs):
        ...
        self.final_up = nn.ConvTranspose3d(f_maps[0], f_maps[0], kernel_size=2, stride=2)
    ...
    def forward(self, x):
        # input = (256, 256, 256)
        x = F.interpolate(x, scale_factor=1/2, mode='trilinear', align_corners=True)
        # encoder part
        encoders_features = []
...
class UNet3D_Transpose(nn.Module):
    """
    3DUnet model from
    `"3D U-Net: Learning Dense Volumetric Segmentation from Sparse Annotation"
        <https://arxiv.org/pdf/1606.06650.pdf>`.
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=64, layer_order='cbe', num_groups=8,
                 **kwargs):
        super(UNet3D_Transpose, self).__init__()
